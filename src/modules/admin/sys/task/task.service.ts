...
import SysTask from './task.entity';

@Provide()
export class AdminSysTaskService extends BaseService {
  @InjectQueue('SysTaskQueue')
  sysTaskQueue: BullQueue;
  ...
  async initTask(): Promise<void> {
    ...
    // 查找所有需要运行的任务
    const tasks = await this.sysTask.find({
      where: {
        status: 1,
      },
    });
  ...
  async info(id: number): Promise<SysTask> {
    return await this.sysTask.findOne({
      where: {
        id,
      },
    });
  ...
  async updateTaskCompleteStatus(tid: number): Promise<void> {
    ...
    const task = await this.sysTask.findOne({
      where: {
        id: tid,
      },
    });
